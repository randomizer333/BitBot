var autobahn = require('autobahn');		//begining of API,do have node.js
...
connection.open();				//eno of API

var coin = [ BTC, ETC, USDT, XRP ]; 		//select coin//
var market = [ BTC_ETC, BTC_USDT,...  ]		//select market
marketvar oldRate;
var newRate;
var oldRate;
var difPercent = 0.1;
var time =;
var boughtRate =;
var sellRate = boughtRate * difPercent;
var balances = marketEvent(returnBalances);	//returns {"BTC":"0.59098578","LTC":"3.31117268", ... }
var roi = ( newRate / oldRate ); 	//check trend ROI = ( (Earnings - Initial Invested Amount) / Initial Invested Amount ) x 100


if		//if falling sell
( roi < 0 + difPercent ){
marketEvent( currencyPair[i], "rate", "amount"); 	//buy
}
else		//if rising buy
( roi < 0 + difPercent ){
marketEvent("currencyPair", "rate", "amount");		//sell
}
